...
export const Profile = () => {
  ...
  const [myBlogs, setMyBlogs] = useState<string | null>(null)
  const [isBanner, setIsBanner] = useState<boolean>(true)
  const getUserBlogs = async () => {
    const response = await axios('')
    setMyBlogs(JSON.stringify(response.data))
  ...
  return (
    <Screen>
      ...
      <div className="flex flex-col items-center justify-center w-full mt-16">
        <img src={userContext?.user?.photo} className="mt-8 mb-2 rounded-full w-36 h-36"></img>  
        <p className="p-2 text-3xl font-bold text-white">{userContext?.user?.displayname}</p>
